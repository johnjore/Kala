    ...
    public partial class Widgets
    {
        public static void Gauge_Group(Grid grid, string x1, string y1, string x2, string y2, string x3, string y3, string header, JArray data)
        {
            ...
            try
            {
                ...
                Grid t_grid = Create_GaugeHeader(header, sx, rx, ry);
                ...
                for (int j = 1; j <= items.Count / rx; j++)
                {
                    ...
                    {
                        CrossLogger.Current.Error("Gauge", "Position x=" + i.ToString() + ", y=" + j.ToString());
                        Create_Gauge(t_grid, item_counter, items[item_counter], i, j);
                        item_counter++;
                    }
                }
                CrossLogger.Current.Error("Gauge", "End Of Position. Items: " + items.Count.ToString());
        ...
        private static void Gauge_Group(Grid grid, string x1, string y1, string x2, string y2, string x3, string y3, string header, JObject data)
        {
            int.TryParse(x1, out int px);
            int.TryParse(y1, out int py);
            int.TryParse(x2, out int sx);
            ...
            try
            {
                ...
                Grid t_grid = Create_GaugeHeader(header, 1, rx, ry);
